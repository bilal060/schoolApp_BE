...
exports.login = catchAsync(async (req, res, next) => {
  ...
  let user;
  user = await User.findOne({ email }).select('+password +verified')
  if (!user) {
...
exports.otpverify = catchAsync(async (req, res, next) => {
  ...
  if (!(email, otp)) {
   ...
   if(!user){
    let studentUsers;
    studentUsers = await studenUser.findOne({email}).select('+password')
    if(!studentUsers){
      return next(new AppError('no StudentUser exist in db ', 401)) 
    }
    const checkStudentOTP = await studentUsers.correctotp(otp, studentUsers.otp)
    if(!checkStudentOTP){
      return next(new AppError('otp not matched', 401)) 
    }
    if(studentUsers.otpExpireTime < Date.now()){
      return next(new AppError('OTP is invalid and expire', 400))
    }
      studentUsers.verified = true;
      studentUsers.otp = undefined;
      studentUsers.otpExpireTime = undefined;
      await studentUsers.save();
     return res.status(200).json({
        ValidUser:{
          email:studentUsers.email,
          verified:studentUsers.verified
        }
      })
   }
   if(user && !user.otp){
    ...
    const checkStudentOTP = await user.correctotp(otp, studentUsers.otp)
    if(!checkStudentOTP){
      return next(new AppError('otp not matched', 401)) 
    }
    if(studentUsers.otpExpireTime < Date.now()){
